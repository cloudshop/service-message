...
public class Shard { // S类封装了机器节点的信息 ，如name、password、ip、port等
    ...
    public Shard(List<NodeVo> shards) {
        super();
        this.shards = shards;
        init();
    }
...
//            System.out.println("hash("+lo+")连接到主机->"+treeKey.get(lo));
    ...
    private void init() { // 初始化一致性hash环
        TreeMap<Long, NodeVo> nodestemp = new TreeMap<Long, NodeVo>();
        TreeMap<Long, NodeVo> treeKeytemp = new TreeMap<Long, NodeVo>();
        for (int i = 0; i != shards.size(); ++i) { // 每个真实机器节点都需要关联虚拟节点
            final NodeVo shardInfo = shards.get(i);

            for (int n = 0; n < NODE_NUM; n++)
                // 一个真实机器节点关联NODE_NUM个虚拟节点
                nodestemp.put(hash("SHARD-" + shardInfo.name + "-NODE-" + n), shardInfo);
        }
        nodes=nodestemp;
        treeKey=treeKeytemp;
    }
...
//        System.out.println(key+"（hash："+hash(key)+"）连接到主机->"+obj.toString());
    ...
    //映射key到真实节点
    public String keyToNodeHostIdentity(String key){
        try {
            SortedMap<Long, NodeVo> tail = nodes.tailMap(hash(key)); // 沿环的顺时针找到一个虚拟节点
            if (tail.size() == 0) {
                return "";
